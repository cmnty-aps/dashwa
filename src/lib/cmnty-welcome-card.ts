...
async function loadAvatarSafe(avatarUrl: string | null): Promise<Image | null> {
...
function drawHexagonPath(ctx: any, x: number, y: number, r: number) {
  ctx.beginPath();
  for (let i = 0; i < 6; i++) {
...
export async function createWideDiscordCard(username: string, avatarUrl: string | null, groupName: string, memberCount: number) {
  const width = 1024;
  const height = 450;
  ...
  if (ctx.roundRect) {
    ...
    drawRoundedRect(ctx, textX, 120, 140, 36, 18);
  }
  ctx.fill();
...
export async function createGoodbyeCard(username: string, avatarUrl: string | null, groupName: string, memberCount: number) {
  const width = 1024;
  const height = 450;
  const canvas = createCanvas(width, height);
  const ctx = canvas.getContext("2d");
...
export async function createWelcomeCardV4(username: string, avatarUrl: string|null, groupName: string, memberCount: number) {
  const width = 1024;
  const height = 450;
  const canvas = createCanvas(width, height);
  const ctx = canvas.getContext("2d");
  ...
  if (ctx.roundRect) {
    // @ts-ignore
    ctx.roundRect(textStart, tagY, tagWidth, 40, 20);
  } else {
...
export async function createGoodbyeCardV4(username: string, avatarUrl: string|null, groupName: string, memberCount: number) {
  const width = 1024;
  const height = 450;
  const canvas = createCanvas(width, height);
  const ctx = canvas.getContext("2d");
